    ...
    <div class="row">
        ...
        <div class="col">
            <div class="card border-secondary">
                <div class="card-body">
                    <h4 class="card-title">Create a Learn day</h4>
                    @if ($errors->any())
                        <div class="mb-3 mt-3">
                    ...
                    <form action="{{ route('learnDays.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('POST')
                        <div class="mb-3">
                            <label for="title" class="form-label">Title</label>
                            <input type="text" name="title" id="title" class="form-control @if($errors->has('title')) border border-danger @endif"
                                placeholder="Title" aria-describedby="helpId" value="">

                            @if ($errors->has('title'))
                                <small class="text-danger">{{ $errors->first('title') }}</small>
                            @else
                                <small id="helpId" class="text-muted">The title of the learn day to be created.</small>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="date" id="date" class="form-control"
                                aria-describedby="helpId" value="date">
